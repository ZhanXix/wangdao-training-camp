 ...
 * 2.0 2019/7/5 编写进程池退出机制，确保发送信号后，父进程依次杀掉子进程，然后自己退出
 */
...
int main(int argc,char *argv[])
{
    ...
    pEvents=(struct epoll_event*)calloc(processNum+2,sizeof(struct epoll_event));//这里1.0版本貌似写错了，1.0版本里calloc第一个参数应该是processNum+1，我写成了processNum
    int readyFdCount=0;
    int busyFlag;
    char exitFlag;
    while(1)
    {
        ...
        for(i=0;i<readyFdCount;i++)
        {
            if(pEvents[i].data.fd==fds_exit[0])//收到退出信号
            {
                ...
                {
                    kill(pChildProcessData[j].pid,SIGUSR1);
                #ifdef DEBUG
                    printf("已向子进程%d发送信号SIGUSR1\n",pChildProcessData[j].pid);
                #endif // DEBUG
                }
                printf("退出处理完毕\n");
                exit(0);
