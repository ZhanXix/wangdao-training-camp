...
void *Thread_Func(void *p)
{
    ...
    while(1)
    {
        ...
        while(0!=recv(clientFd,&fileData.dataLen,4,0))
        {
            ...
            ret=Command_Deal(fileData.dataBuf,clientFd,&code);
            if(-1==ret)
            {
                break;
            }
...
int Command_Deal(char *command,int clientFd,int *pCode)
{
    MYSQL *connect=NULL;
    putchar('\n');
    int ret;
    if(strcmp(command,"cd")==0)//cd
    {
        ...
        puts("捕捉命令：mkdir");
        ret=Command_mkdir(clientFd,pCode,connect);
        ERROR_RETURN(ret);
    }else if(strcmp(command,"rmdir")==0){//rmdir
        puts("捕捉命令：rmdir");
        ret=Command_rmdir(clientFd,pCode,connect);
        ERROR_RETURN(ret);
    }
    return 0;
}
...
int Command_mkdir(int clientFd,int *pCode,MYSQL *connect)
{
    CONNECT_MYSQL;
    int ret;
    int code=*pCode;
    ...
    if(!fork())//创建子进程以进行创建文件夹
    {
        ...
        //插入数据库
        bzero(query,sizeof(query));
        sprintf(query,"insert into file_list (precode,filename,filesize,filetype,owner) values (%d,'%s',4096,'d','xiaoming');",code,dirname);
        printf("mkdir:insert into file_list (precode,filename,filesize,filetype,owner) values (%d,'%s',4096,'d','xiaoming');\n",code,dirname);
        if(mysql_query(connect,query))
        {
...
int Command_rmdir(int clientFd,int *pCode,MYSQL *connect)
{
    CONNECT_MYSQL;
    int ret;
    int code=*pCode;
    ...
    strcpy(dirname,fileData.dataBuf);
    sprintf(query,"select * from file_list where precode=%d and filename='%s' and filetype='d';",code,dirname);
    printf("rmdir:select * from file_list where precode=%d and filename='%s' and filetype='d';\n",code,dirname);
    if(mysql_query(connect,query)){
