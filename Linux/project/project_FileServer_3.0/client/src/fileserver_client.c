...
int SEND_GETS(int socketFd,char *buf,int i)
{
    ...
    while((dir=readdir(dirFd)))
    {
        if(strcmp(dir->d_name,filename)==0)
        {
            ...
            {
                printf(LIGHT_BLUE "Do you want to continue gets it?\n");
                printf(NONE "(y/n):");
                fflush(stdout);
                read(STDIN_FILENO,sendBuf,sizeof(sendBuf));
                if(sendBuf[0]=='y'&&sendBuf[1]=='\n'){//续传原文件
                    ...
                    ret=stat(dir->d_name,&state_buf);
                    ERROR_CHECK(ret,-1,"stat");
                    myfilesize=state_buf.st_size;
                    printf("now filesize=%ld\n",myfilesize);
                    break;
                }else{
                    bzero(&fileData,sizeof(fileData));
                    strcpy(fileData.dataBuf,"broken");
                    SEND_FILEDATA;
                    return 0;
                }
            }
    ...
    {
        bzero(&fileData,sizeof(fileData));
        strcpy(fileData.dataBuf,"continue");
        SEND_FILEDATA;
        bzero(&fileData,sizeof(fileData));
        memcpy(fileData.dataBuf,&myfilesize,sizeof(myfilesize));
        fileData.dataLen=sizeof(myfilesize);
        memcpy(sendBuf,&fileData,fileData.dataLen+4);
        ret=send(socketFd,sendBuf,fileData.dataLen+4,0);
        ERROR_CHECK(ret,-1,"send");
        //得到是否可以续传的信息
        GET_FILEDATA;
        if(strcmp(fileData.dataBuf,"error")==0){
            printf("gets ERROR: this file cannot continue to be gets! You should delete it first\n");
            return 0;
        }
    }else{
        bzero(&fileData,sizeof(fileData));
        strcpy(fileData.dataBuf,"gets");
        SEND_FILEDATA;
    }
    //接收文件大小
    off_t fileSize=0;
...
int SEND_REMOVE(int socketFd,char *buf,int i)
{
    ...
    if(strcmp(fileData.dataBuf,"error")==0)
    {
        printf("remove ERROR: no same file or dir exist in server\n");
        return 0;
    }else{
        GET_FILEDATA;
        if(strcmp(fileData.dataBuf,"error")==0)
        {
            printf(LIGHT_BLUE "This dir is not empty！\n");
            printf("Do you want to delete all file in the dir?\n");
            printf(NONE "(y/n):");
            fflush(stdout);
            char readBuf[100]={0};
            read(STDIN_FILENO,readBuf,sizeof(readBuf));
            bzero(&fileData,sizeof(fileData));
            if(readBuf[0]=='y'&&readBuf[1]=='\n')
            {
                strcpy(fileData.dataBuf,"Y");
                SEND_FILEDATA;
                GET_FILEDATA;
                if(strcmp(fileData.dataBuf,"error")==0)
                {
